...
var done_rec_box,myconnects,uid;
function getAllElements(){
	...
	myconnects = document.getElementById('myconnects');	
			...
			if(resp_.length!=0){
				...
				for(i=0;i<resp_.length;i++){
					...
					if(resp_[i].fromUser.userId==uid.value){
						op.value = resp_[i].toUser.userName;
					}else{
						op.value = resp_[i].fromUser.userName;
					}
					
					myconnects.appendChild(op);
				}
			}else{
